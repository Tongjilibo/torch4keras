...
## 4. 版本说明
- **v0.1.3**: 20230828 compile()可不传参，interval不一致报warning, 去除部分self.vars, 调整move_to_model_device逻辑，方便下游集成, DDP每个epoch重新设置随机数，save_weights()和load_weights()可以按照`pretrain`格式
...
## 5. 更新：
- **20230828**: compile()可不传参，interval不一致报warning, 去除部分self.vars, 调整move_to_model_device逻辑，方便下游集成, DDP每个epoch重新设置随机数，save_weights()和load_weights()可以按照`pretrain`格式
